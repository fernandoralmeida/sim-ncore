...
INSERT INTO [__EFMigrationsHistory] ([MigrationId], [ProductVersion])
VALUES (N'20230427144830_m_fix_anonimo', N'6.0.8');
GO
...
FROM [sys].[default_constraints] [d]
INNER JOIN [sys].[columns] [c] ON [d].[parent_column_id] = [c].[column_id] AND [d].[parent_object_id] = [c].[object_id]
WHERE ([d].[parent_object_id] = OBJECT_ID(N'[Atendimento]') AND [c].[name] = N'Anonimo');
IF @var10 IS NOT NULL EXEC(N'ALTER TABLE [Atendimento] DROP CONSTRAINT [' + @var10 + '];');
ALTER TABLE [Atendimento] ALTER COLUMN [Anonimo] bit NULL;
GO

INSERT INTO [__EFMigrationsHistory] ([MigrationId], [ProductVersion])
VALUES (N'20230427145446_m_fix_anonimo_1', N'6.0.8');
GO
